...
def main():
  ...
  for town_ in towns:
    ...
    for tl_ in traffic_lights_list:
      ...
      grp_tl_locs = [
          (round(gtl_.get_transform().location.x, 2), round(gtl_.get_transform().location.y, 2)) for gtl_ in grp_tl
      ]
    ...
    signalized_junctions_bbs = [(junc_.bounding_box.location.x, junc_.bounding_box.location.y)
    ...
    if len(signalized_junctions):
            ...
            for j_wp in j_wps:
              ...
              for j_, wps_ in enumerate(truncated_wp_lst):
                if not are_loc_dups[j_]:
                  ...
                  scan_route_for_scenarios(carla_map.name, extended_route, scenarios_list[key_])
      ...
      if args.save_dir is not None and len_tree > 0:
        tree_7.write(args.save_file['s7'], xml_declaration=True, encoding='utf-8', pretty_print=True)
      ...
      if args.save_dir is not None and len_tree > 0:
        tree_8.write(args.save_file['s8'], xml_declaration=True, encoding='utf-8', pretty_print=True)
      ...
      if args.save_dir is not None and len_tree > 0:
        tree_9.write(args.save_file['s9'], xml_declaration=True, encoding='utf-8', pretty_print=True)
